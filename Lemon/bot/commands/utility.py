...
class UtilityCommands:
    ...
    async def setup_commands(self):
        ...
        async def info(interaction: discord.Interaction):
            embed = discord.Embed(
                title="🤖 Leo",
                description="**A Discord bot with music, moderation, and fun commands!**",
                color=COLORS['info']
            )
            ...
            links_value = ""
            if BOT_INVITE_LINK:
                links_value += f"📨 [**Invite Me**]({BOT_INVITE_LINK})\n"
            if SUPPORT_SERVER_LINK:
                links_value += f"🛠️ [**Support Server**]({SUPPORT_SERVER_LINK})"
            ...
            embed.set_footer(text="Made with ❤️ by Peeyoosh")
...
class HelpView(discord.ui.View):
    ...
    def create_main_embed(self):
        embed = discord.Embed(
            title="🌟 Leo Command Center 🌟",
            description="✨ **Welcome to Leo's amazing world!** ✨\n\n🚀 **Choose a category below to explore all the incredible features!**\n\n💫 *Click any button to discover what Leo can do for your server!*",
            color=0x7289da  # Discord blurple
        )
        
        embed.add_field(
            name="🎵 Music & Audio",
            value="🎶 Play your favorite songs from YouTube\n🎧 Queue management & controls\n🔊 High-quality audio streaming",
            inline=True
        )
        
        embed.add_field(
            name="🛡️ Server Moderation", 
        ...
        embed.set_footer(text="🌈 Click the colorful buttons below to explore each category! 🌈")
